...
use paniq::socks5::{AuthConfig, RelayConnector, Socks5Server, TargetAddr, SocksError, IoStream};
...
fn log(msg: &str) {
    let now = std::time::SystemTime::now()
        ...
        .unwrap_or_default();
    eprintln!("[{}.{:03}] socks5d: {}", now.as_secs(), now.subsec_millis(), msg);
}
...
fn build_client_config_from_profile(_profile: &Profile) -> Result<quinn::ClientConfig, Box<dyn std::error::Error>> {
...
impl QuicConnector {
    async fn connect(&self, target: &TargetAddr) -> Result<Box<dyn IoStream + Send>, String> {
        ...
        if elapsed > std::time::Duration::from_millis(100) {
            log(&format!("[QuicConnector] connect: open_bi took slow: {:?}", elapsed));
        }
        ...
        log(&format!("[QuicConnector] connect: Proxy request sent, returning BiStreamWrapper for {:?}", target));
