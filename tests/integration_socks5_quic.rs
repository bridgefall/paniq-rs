...
async fn integration_socks5_over_quic() {
                ...
                tokio::spawn(async move {
                    while let Ok((mut send, mut recv)) = conn.accept_bi().await {
                        tokio::spawn(async move {
                            ...
                            // Connect to target
                            if let Ok(mut target_stream) = tokio::net::TcpStream::connect(&target_str).await {
    ...
    let quinn_conn = connect_after_handshake(
        client_sock,
        server_addr,
        client_framer,
        client_config,
    ...
    socks_conn.write_all(&[0x05, 0x02, 0x00, 0x02]).await.unwrap(); // ver, nmethods, no-auth, userpass
    let mut resp = [0u8; 2];
