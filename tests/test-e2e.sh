...
HTTP_LOG="$LOG_DIR/http-server-$TIMESTAMP.log"
...
cleanup() {
    ...
    if [ -n "$HTTP_PID" ] && kill -0 "$HTTP_PID" 2>/dev/null; then
        echo "Stopping HTTP server (PID: $HTTP_PID)" | tee -a "$TEST_LOG"
...
echo "=== Starting local HTTP server ===" | tee -a "$TEST_LOG"
# Create a simple test directory with a country code file
TEST_HTTP_DIR="$LOG_DIR/http-root"
mkdir -p "$TEST_HTTP_DIR"
echo "US" > "$TEST_HTTP_DIR/country_code"

# Start Python HTTP server binding to IPv4 explicitly
cd "$TEST_HTTP_DIR"
python3 -m http.server "$HTTP_PORT" --bind 127.0.0.1 > "$HTTP_LOG" 2>&1 &
HTTP_PID=$!
cd "$SCRIPT_DIR"
echo "HTTP server PID: $HTTP_PID" | tee -a "$TEST_LOG"

# Wait longer for HTTP server to be ready
sleep 2

# Verify HTTP server is actually listening
if ! curl -s -m 1 "http://127.0.0.1:$HTTP_PORT/country_code" > /dev/null 2>&1; then
    echo "⚠️ WARNING: HTTP server not responding yet, waiting longer..." | tee -a "$TEST_LOG"
    sleep 2
    if ! curl -s -m 1 "http://127.0.0.1:$HTTP_PORT/country_code" > /dev/null 2>&1; then
        echo "❌ FAILED: HTTP server not responding" | tee -a "$TEST_LOG"
        cat "$HTTP_LOG" | tee -a "$TEST_LOG"
        exit 1
    fi
fi

echo "✅ HTTP server is ready on port $HTTP_PORT" | tee -a "$TEST_LOG"
...
echo "Testing: curl -m 30 -vvv --socks5-hostname 127.0.0.1:$SOCKS_PORT http://localhost:$HTTP_PORT/country_code" | tee -a "$TEST_LOG"

# Run curl and capture output
set +e
CURL_OUTPUT=$(curl -m 30 -vvv --socks5-hostname "127.0.0.1:$SOCKS_PORT" "http://localhost:$HTTP_PORT/country_code" 2>&1)
CURL_EXIT=$?
set -e
...
if [ $CURL_EXIT -eq 0 ]; then
    RESPONSE=$(echo "$CURL_OUTPUT" | tail -1)
    echo "✅ SUCCESS: Curl completed successfully" | tee -a "$TEST_LOG"
    echo "Response received: $RESPONSE" | tee -a "$TEST_LOG"

    # Validate response contains "US"
    if echo "$RESPONSE" | grep -q "US"; then
        echo "✅ Correct response received" | tee -a "$TEST_LOG"
    else
        echo "⚠️  WARNING: Unexpected response content" | tee -a "$TEST_LOG"
    fi
else
