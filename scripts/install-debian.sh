...
# Helper function for fresh install
first_time_install() {
...
update_service() {
    echo -e "${COLOR_BLUE}Restarting paniq-rs-proxy service...${COLOR_RESET}"
    if systemctl is-active --quiet paniq-rs-proxy.service; then
        systemctl restart paniq-rs-proxy.service
    else
        echo -e "${COLOR_BLUE}Service not active, simply skipping restart.${COLOR_RESET}"
    fi
}
